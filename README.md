...
- **월 이동은 자동입니다.** 오늘 날짜를 기준으로 매번 [이전전달·이전달·이번달·다음달]을 다시 계산하기 때문에, 다음 달이 되면 자동으로 화면에 보이는 4개월 범위가 한 칸씩 밀려납니다.
- **화면 범위보다 오래된 기록은 매일 자정 DB에서 자동으로 삭제됩니다** (Supabase의 pg_cron 예약 작업 `cleanup-old-transactions`). 한 번 지워지면 복구할 수 없으니, 이 동작을 끄고 싶다면 Supabase SQL Editor에서 `select cron.unschedule('cleanup-old-transactions');` 를 실행하면 됩니다. (자세한 내용은 `supabase/schema.sql` 맨 아래 참고)
